...
@section('content')
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6">
            ...
            <h1 class="text-3xl font-bold text-slate-900">Edit Jadwal Latihan</h1>
        </div>

        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <form action="{{ route('admin.training-schedules.update', $trainingSchedule) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <select name="club_id"
                            ...
                            @foreach ($clubs as $club)
                                <option value="{{ $club->id }}"
                                    {{ old('club_id', $trainingSchedule->club_id) == $club->id ? 'selected' : '' }}>
                                    {{ $club->name }}</option>
                            @endforeach
                        </select>
                        @error('club_id')
                        ...
                        <select name="training_location_id"
                            ...
                            @foreach ($trainingLocations as $location)
                                <option value="{{ $location->id }}"
                                    {{ old('training_location_id', $trainingSchedule->training_location_id) == $location->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="day_of_week"
                            ...
                            @foreach (['monday' => 'Senin', 'tuesday' => 'Selasa', 'wednesday' => 'Rabu', 'thursday' => 'Kamis', 'friday' => 'Jumat', 'saturday' => 'Sabtu', 'sunday' => 'Minggu'] as $key => $label)
                                <option value="{{ $key }}"
                                    {{ old('day_of_week', $trainingSchedule->day_of_week) === $key ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                        @error('day_of_week')
                    ...
                    <div>
                        ...
                        <select name="activity_type"
                            ...
                            <option value="sekolah"
                                {{ old('activity_type', $trainingSchedule->activity_type) === 'sekolah' ? 'selected' : '' }}>
                                Sekolah</option>
                            <option value="luar_sekolah"
                                {{ old('activity_type', $trainingSchedule->activity_type) === 'luar_sekolah' ? 'selected' : '' }}>
                                Luar Sekolah</option>
                        </select>
                        @error('activity_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="time" name="start_time"
                            value="{{ old('start_time', $trainingSchedule->start_time) }}"
                            class="mt-1 w-full rounded-md border-slate-300 focus:border-indigo-500 focus:ring-indigo-500"
                            required>
                        @error('start_time')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="time" name="end_time" value="{{ old('end_time', $trainingSchedule->end_time) }}"
                            class="mt-1 w-full rounded-md border-slate-300 focus:border-indigo-500 focus:ring-indigo-500"
                            required>
                        @error('end_time')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="status"
                            class="mt-1 w-full rounded-md border-slate-300 focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="active"
                                {{ old('status', $trainingSchedule->status) === 'active' ? 'selected' : '' }}>Active
                            </option>
                            <option value="inactive"
                                {{ old('status', $trainingSchedule->status) === 'inactive' ? 'selected' : '' }}>Inactive
                            </option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
