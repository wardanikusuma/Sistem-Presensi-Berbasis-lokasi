...
@section('content')
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6">
            ...
            <h1 class="text-3xl font-bold text-slate-900">Tambah Sesi Latihan</h1>
        </div>

        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <form action="{{ route('admin.training-sessions.store') }}" method="POST" class="space-y-5">
                @csrf
                        ...
                        <select name="training_schedule_id"
                            ...
                            @foreach ($trainingSchedules as $trainingSchedule)
                                <option value="{{ $trainingSchedule->id }}"
                                    {{ old('training_schedule_id') == $trainingSchedule->id ? 'selected' : '' }}>
